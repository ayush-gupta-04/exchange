...
async function placeBuyOrder({market,basePrice,spread} : {market : string,basePrice : number,spread :number}){
...
async function placeSellOrder({market,basePrice,spread} : {market : string,basePrice : number,spread :number}){
    const price = (basePrice + (Math.random() * spread * 2 - spread)).toFixed(1);
