        ...
        this.ws.on("message" , (message : string) => {
            ...
            try {
                if(parsedMessage.method == "SUBSCRIBE"){
                    parsedMessage.params.forEach(channel => SubscriptionManager.getInstance().subscribe(this.userId,channel))
                }else{
                    parsedMessage.params.forEach(channel => SubscriptionManager.getInstance().unsubscribe(this.userId,channel))
                }
            } catch (error) {
                console.log(error)
            } 
